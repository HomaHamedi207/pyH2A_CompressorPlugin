...
import pytest
import numpy as np
from pyH2A.Plugins.Hourly_Irradiation_Plugin import Hourly_Irradiation_Plugin, import_hourly_data

class DummyDCF:
    """DCF object for Hourly_Irradiation_Plugin with configurable inputs."""
    def __init__(self, hourly_file, module_tilt, array_azimuth, nominal_temp,
                 mismatch_derating, dirt_derating, temp_coeff):
        self.inp = {
            "Hourly Irradiation": {
                "File": {"Value": hourly_file}
            },
...
def test_hourly_irradiation_plugin(case):
    ...
    assert np.sum(plugin.power_dat_kW)/365. == pytest.approx(expected["mean_power_dat_kW"])
    assert np.sum(plugin.power_kW)/365. == pytest.approx(expected["mean_power_kW"])
    assert np.sum(plugin.power_sat_kW)/365. ==  pytest.approx(expected["mean_power_sat_kW"])
