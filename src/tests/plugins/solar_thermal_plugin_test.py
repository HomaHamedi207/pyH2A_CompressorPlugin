...
class DummyDCF:
    """DummyDCF for Solar Thermal Plugin for unit testing"""
    def __init__(
        self,
        ...
        self.inp = {
            "Technical Operating Parameters and Specifications": {"Design Output per Day": {"Value": design_output_per_day}},
            "Solar-to-Hydrogen Efficiency": {"STH (%)": {"Value": sth_efficiency}},
            "Solar Input": {"Mean solar input (kWh/m2/day)": {"Value":mean_solar_input }},
            "Non-Depreciable Capital Costs": {"Additional Land Area (%)": {"Value": additional_land_area}}
        }
...
def test_solar_thermal_plugin(case):
    ...
    assert plugin.area_acres == case["expected"]
